...
// 1048..1063: Phase 5 extras (16)
// 1064..1066: Phase 6 extras (weakSquaresMG, weakKingSquaresMG, tempo)
...
func formatArray6x64(vals [6][64]int) string {
    ...
    for pi := 0; pi < 6; pi++ {
        ...
        for i := 0; i < 64; i++ {
            if i%8 == 0 { b.WriteString("\t\t\t") }
            b.WriteString(fmt.Sprintf("%d,", vals[pi][i]))
            if i%8 == 7 { b.WriteString("\n") }
        }
        b.WriteString("\t\t},\n")
    }
    return b.String()
...
func formatArray100(vals []int) string {
    var b strings.Builder
    for i := 0; i < 100; i++ {
        if i%10 == 0 { b.WriteString("\t\t") }
        b.WriteString(fmt.Sprintf("%d,", vals[i]))
        if i%10 == 9 { b.WriteString("\n") }
    }
    return b.String()
}
...
func main() {
    ...
    // Phase 2
    var dMG, dEG, iMG, iEG, cMG, cEG, phMG, phEG, bMG, bEG, plMG, plEG, bwMG, bwEG int
    // Mobility
    ...
    if len(m.Theta) > 0 {
        ...
        // PST MG
        for pt := 0; pt < 6; pt++ {
            for i := 0; i < 64; i++ { if idx < len(th) { pstMG[pt][i] = rd(th[idx]); idx++ } }
        }
        // PST EG
        for pt := 0; pt < 6; pt++ {
            for i := 0; i < 64; i++ { if idx < len(th) { pstEG[pt][i] = rd(th[idx]); idx++ } }
        }
        // Material MG/EG
        for i := 0; i < 6 && idx < len(th); i++ { matMG[i] = rd(th[idx]); idx++ }
        for i := 0; i < 6 && idx < len(th); i++ { matEG[i] = rd(th[idx]); idx++ }
        // Passers MG/EG
        for i := 0; i < 64 && idx < len(th); i++ { passMG[i] = rd(th[idx]); idx++ }
        for i := 0; i < 64 && idx < len(th); i++ { passEG[i] = rd(th[idx]); idx++ }
        // Phase 1 (8)
        if idx+8 <= len(th) {
            bpMG = rd(th[idx+0]); bpEG = rd(th[idx+1]); rsMG = rd(th[idx+2]); roMG = rd(th[idx+3])
            srEG = rd(th[idx+4]); qcEG = rd(th[idx+5]); qiMG = rd(th[idx+6]); qiEG = rd(th[idx+7])
        }
        idx += 8
        // Phase 2 (14)
        if idx+14 <= len(th) {
            dMG = rd(th[idx+0]); dEG = rd(th[idx+1]); iMG = rd(th[idx+2]); iEG = rd(th[idx+3])
            cMG = rd(th[idx+4]); cEG = rd(th[idx+5]); phMG = rd(th[idx+6]); phEG = rd(th[idx+7])
            bMG = rd(th[idx+8]); bEG = rd(th[idx+9]); plMG = rd(th[idx+10]); plEG = rd(th[idx+11])
            bwMG = rd(th[idx+12]); bwEG = rd(th[idx+13])
        }
        idx += 14
        // Mobility MG/EG (7 + 7)
        for i := 0; i < 7 && idx < len(th); i++ { mobMG[i] = rd(th[idx]); idx++ }
        for i := 0; i < 7 && idx < len(th); i++ { mobEG[i] = rd(th[idx]); idx++ }
        // King safety table (100)
        for i := 0; i < 100 && idx < len(th); i++ { ks[i] = rd(th[idx]); idx++ }
        // King correlates (4)
        if idx+4 <= len(th) { kSemi = rd(th[idx+0]); kOpen = rd(th[idx+1]); kMinor = rd(th[idx+2]); kPawn = rd(th[idx+3]) }
        idx += 4
        // Extras (16)
        if idx+16 <= len(th) {
            ex_knOutMG = rd(th[idx+0]); ex_knOutEG = rd(th[idx+1]); ex_knThrMG = rd(th[idx+2]); ex_knThrEG = rd(th[idx+3])
            ex_stackMG = rd(th[idx+4]); ex_xrayMG = rd(th[idx+5]); ex_connMG = rd(th[idx+6]); ex_biOutMG = rd(th[idx+7])
            ex_bxKMG = rd(th[idx+8]); ex_bxRMG = rd(th[idx+9]); ex_bxQMG = rd(th[idx+10])
            ex_stormMG = rd(th[idx+11]); ex_proxMG = rd(th[idx+12]); ex_levStormMG = rd(th[idx+13])
            // idx+14, idx+15 are tuner-only (center mobility deltas); ignore for engine output
        }
        idx += 16
        // Phase 6 (3)
        if idx+3 <= len(th) { weakMG = rd(th[idx+0]); weakKingMG = rd(th[idx+1]); tempo = rd(th[idx+2]) }
    } else {
        // Grouped fields path (fallback)
        if m.PST != nil {
            for pt := 0; pt < 6; pt++ {
                for i := 0; i < 64; i++ { pstMG[pt][i] = rd(m.PST.MG[pt][i]); pstEG[pt][i] = rd(m.PST.EG[pt][i]) }
            }
        }
        if len(m.MaterialMG) == 6 { for i := 0; i < 6; i++ { matMG[i] = rd(m.MaterialMG[i]) } }
        if len(m.MaterialEG) == 6 { for i := 0; i < 6; i++ { matEG[i] = rd(m.MaterialEG[i]) } }
        if len(m.PassersMG) == 64 { for i := 0; i < 64; i++ { passMG[i] = rd(m.PassersMG[i]) } }
        if len(m.PassersEG) == 64 { for i := 0; i < 64; i++ { passEG[i] = rd(m.PassersEG[i]) } }
        if m.BishopPairMG != nil { bpMG = rd(*m.BishopPairMG) }
        if m.BishopPairEG != nil { bpEG = rd(*m.BishopPairEG) }
        if m.RookSemiOpenFileMG != nil { rsMG = rd(*m.RookSemiOpenFileMG) }
        if m.RookOpenFileMG != nil { roMG = rd(*m.RookOpenFileMG) }
        if m.SeventhRankEG != nil { srEG = rd(*m.SeventhRankEG) }
        if m.QueenCentralizedEG != nil { qcEG = rd(*m.QueenCentralizedEG) }
        if m.QueenInfiltrationMG != nil { qiMG = rd(*m.QueenInfiltrationMG) }
        if m.QueenInfiltrationEG != nil { qiEG = rd(*m.QueenInfiltrationEG) }
        if len(m.MobilityMG) == 7 { for i := 0; i < 7; i++ { mobMG[i] = rd(m.MobilityMG[i]) } }
        if len(m.MobilityEG) == 7 { for i := 0; i < 7; i++ { mobEG[i] = rd(m.MobilityEG[i]) } }
        if len(m.KingSafety) == 100 { for i := 0; i < 100; i++ { ks[i] = rd(m.KingSafety[i]) } }
    }
    ...
    if err := os.MkdirAll(filepath.Dir(*outPath), 0o755); err != nil { panic(err) }
    if err := os.WriteFile(*outPath, []byte(out.String()), 0o644); err != nil { panic(err) }
}
