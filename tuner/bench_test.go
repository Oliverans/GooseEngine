...
func BenchmarkMain(b *testing.B) {
    ...
    if v := os.Getenv("TUNER_BENCH_ROWS"); v != "" {
        if n, err := strconv.Atoi(v); err == nil && n > 0 { rows = n }
    }
    // Default batch 65536 to mirror your CLI runs
    batch := 65536
    if v := os.Getenv("TUNER_BENCH_BATCH"); v != "" {
        if n, err := strconv.Atoi(v); err == nil && n > 0 { batch = n }
    }
    ...
    if len(data) == 0 { b.Fatalf("no data for benchmark") }
    // Prebuild boards to avoid parsing in the timed section
    boards := make([]*Position, len(data))
    for i := range data { boards[i] = NewBoardFromSample(data[i]) }
    ...
    for i := 0; i < b.N; i++ {
        ...
        for off := 0; off < len(boards); off += batch {
            ...
            if end > len(boards) { end = len(boards) }
