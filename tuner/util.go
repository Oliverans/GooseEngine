...
func BuildLRScale(fe Featurizer) []float64 {
	...
	for i := 0; i < 8; i++ {
		if p1Enabled[i] {
			scale[le.layout.P1Start+i] = 0.5
		} else {
			scale[le.layout.P1Start+i] = 0.0
		}
	}
	// Pawn structure (16)
	pawnEnabled := [16]bool{
		...
		pickBool(t.WeakLeverMG, t.PawnStruct[12]),
		pickBool(t.WeakLeverEG, t.PawnStruct[13]),
		pickBool(t.BackwardMG, t.PawnStruct[14]),
		pickBool(t.BackwardEG, t.PawnStruct[15]),
	}
	for i := 0; i < 16; i++ {
		if pawnEnabled[i] {
			scale[le.layout.PawnStructStart+i] = 0.5
		} else {
			scale[le.layout.PawnStructStart+i] = 0.0
		}
	}
	...
	for i := 0; i < 7; i++ {
		if mgEnabled[i] {
			scale[le.layout.MobilityMGStart+i] = 0.02
		} else {
			scale[le.layout.MobilityMGStart+i] = 0.0
		}
	}
	for i := 0; i < 7; i++ {
		if egEnabled[i] {
			scale[le.layout.MobilityEGStart+i] = 0.02
		} else {
			scale[le.layout.MobilityEGStart+i] = 0.0
		}
	}
	...
	for i := 0; i < 4; i++ {
		if kc[i] {
			scale[le.layout.KingCorrStart+i] = 0.5
		} else {
			scale[le.layout.KingCorrStart+i] = 0.0
		}
	}
	// Extras (16)
	ex := [16]bool{
	...
	for i := 0; i < 16; i++ {
		if ex[i] {
			scale[le.layout.ExtrasStart+i] = 0.5
		} else {
			scale[le.layout.ExtrasStart+i] = 0.0
		}
	}
	// Imbalance (12)
	imb := [12]bool{
		pickBool(t.ImbKnightPerPawnMG, t.Imbalance[0]),
		pickBool(t.ImbKnightPerPawnEG, t.Imbalance[1]),
		pickBool(t.ImbBishopPerPawnMG, t.Imbalance[2]),
		pickBool(t.ImbBishopPerPawnEG, t.Imbalance[3]),
		pickBool(t.ImbMinorsForMajorMG, t.Imbalance[4]),
		pickBool(t.ImbMinorsForMajorEG, t.Imbalance[5]),
		pickBool(t.ImbRedundantRookMG, t.Imbalance[6]),
		pickBool(t.ImbRedundantRookEG, t.Imbalance[7]),
		pickBool(t.ImbRookQueenOverlapMG, t.Imbalance[8]),
		pickBool(t.ImbRookQueenOverlapEG, t.Imbalance[9]),
		pickBool(t.ImbQueenManyMinorsMG, t.Imbalance[10]),
		pickBool(t.ImbQueenManyMinorsEG, t.Imbalance[11]),
	}
	for i := 0; i < 12; i++ {
		if imb[i] {
			scale[le.layout.ImbalanceStart+i] = 0.5
		} else {
			scale[le.layout.ImbalanceStart+i] = 0.0
		}
	}
	// Weak + tempo (3)
	wt := [3]bool{
	...
	for i := 0; i < 3; i++ {
		if wt[i] {
			scale[le.layout.WeakTempoStart+i] = 0.5
		} else {
			scale[le.layout.WeakTempoStart+i] = 0.0
		}
	}
	return scale
}
