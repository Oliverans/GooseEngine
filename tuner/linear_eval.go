...
func (le *LinearEval) Eval(pos *Position) float64 {
	...
	if le.Toggles.P1Eval {
		...
		for wr := pos.White.Rooks; wr != 0; wr &= wr - 1 {
			if (bits.TrailingZeros64(wr) / 8) == 6 { w7++ }
		}
		for br := pos.Black.Rooks; br != 0; br &= br - 1 {
			if (bits.TrailingZeros64(br) / 8) == 1 { b7++ }
		}
	...
	if le.Toggles.PawnStructEval {
		...
		mg += float64(mgDiffs[6]) * le.BackwardMG
		eg += float64(egDiffs[6]) * le.BackwardEG
		// cache pawn diffs
...
func (le *LinearEval) Grad(pos *Position, scale float64, g []float64) {
    ...
    const extras5 = 16
	const weakTempo = 3
	minLen := egBase + pieceCount*slotsPerPiece + matSlots*2 + passSlots*2 + extraScalars + pawnStructScalars + phase3 + ksSlots + ksCorr + extras5 + weakTempo
	if len(g) < minLen {
		return
	}
	...
	if le.Toggles.PSTTrain {
		for bb := pos.White.Pawns; bb != 0; bb &= bb - 1 { add(P, bits.TrailingZeros64(bb), +1) }
		for bb := pos.White.Knights; bb != 0; bb &= bb - 1 { add(N, bits.TrailingZeros64(bb), +1) }
		for bb := pos.White.Bishops; bb != 0; bb &= bb - 1 { add(B, bits.TrailingZeros64(bb), +1) }
		for bb := pos.White.Rooks; bb != 0; bb &= bb - 1 { add(R, bits.TrailingZeros64(bb), +1) }
		for bb := pos.White.Queens; bb != 0; bb &= bb - 1 { add(Q, bits.TrailingZeros64(bb), +1) }
		for bb := pos.White.Kings; bb != 0; bb &= bb - 1 { add(K, bits.TrailingZeros64(bb), +1) }
		mirror := func(sq int) int { return flipView[sq] }
		for bb := pos.Black.Pawns; bb != 0; bb &= bb - 1 { add(P, mirror(bits.TrailingZeros64(bb)), -1) }
		for bb := pos.Black.Knights; bb != 0; bb &= bb - 1 { add(N, mirror(bits.TrailingZeros64(bb)), -1) }
		for bb := pos.Black.Bishops; bb != 0; bb &= bb - 1 { add(B, mirror(bits.TrailingZeros64(bb)), -1) }
		for bb := pos.Black.Rooks; bb != 0; bb &= bb - 1 { add(R, mirror(bits.TrailingZeros64(bb)), -1) }
		for bb := pos.Black.Queens; bb != 0; bb &= bb - 1 { add(Q, mirror(bits.TrailingZeros64(bb)), -1) }
		for bb := pos.Black.Kings; bb != 0; bb &= bb - 1 { add(K, mirror(bits.TrailingZeros64(bb)), -1) }
	}
	...
	if le.Toggles.P1Train {
		// Bishop pair
		bpDiff := 0.0
		if bits.OnesCount64(pos.White.Bishops) >= 2 { bpDiff += 1 }
		if bits.OnesCount64(pos.Black.Bishops) >= 2 { bpDiff -= 1 }
		...
		// Rooks on 7th (EG)
		w7, b7 := 0, 0
		for wr := pos.White.Rooks; wr != 0; wr &= wr - 1 { if (bits.TrailingZeros64(wr) / 8) == 6 { w7++ } }
		for br := pos.Black.Rooks; br != 0; br &= br - 1 { if (bits.TrailingZeros64(br) / 8) == 1 { b7++ } }
		...
		wInf, bInf := 0, 0
		for q := pos.White.Queens; q != 0; q &= q - 1 { if (bits.TrailingZeros64(q) / 8) >= 5 { wInf++ } }
		for q := pos.Black.Queens; q != 0; q &= q - 1 { if (bits.TrailingZeros64(q) / 8) <= 2 { bInf++ } }
		diffInf := float64(wInf - bInf)
	...
	// Phase 3 gradients (mobility only)
	off += 14
	...
	if le.Toggles.MobilityTrain {
		for pt := 1; pt <= 6; pt++ { g[off+pt] += scale * mgf * float64(mobMG[pt]) }
	}
	off += 7
	if le.Toggles.MobilityTrain {
		for pt := 1; pt <= 6; pt++ { g[off+pt] += scale * egf * float64(mobEG[pt]) }
	}
	off += 7
	...
	if le.Toggles.ExtrasTrain {
		...
		var mMG [7]int
		if le.cache.pos == (*gm.Board)(pos) { mMG = le.cache.mobMG } else { mMG, _, _, _ = eng.MobAtkDiffs((*gm.Board)(pos)) }
		knDelta, biDelta := eng.CenterMobilityScales((*gm.Board)(pos))
...
// Params returns the parameter vector I, in the following layout:
//
//	[ 0 ..  6*64-1]        PST MG (piece-major, then 64 squares)
//	[..  12*64-1]          PST EG (piece-major, then 64 squares)
//	[..  +6-1]             Material MG (P..K)
//	[..  +6-1]             Material EG (P..K)
//	[..  +64-1]            Passed pawn MG (square 0..63 white perspective)
//	[..  +64-1]            Passed pawn EG (square 0..63 white perspective)
//	[..  +8-1]             Phase 1 scalars (see source)
//	[..  +14-1]            Pawn structure scalars (MG/EG pairs)
//	[..  +14-1]            Mobility MG (7) + Mobility EG (7)
//	[..  +100-1]           KingSafetyTable (100)
...
// SetParams replaces I, and updates backing PST and feature weights.
func (le *LinearEval) SetParams(p []float64) {
    if le == nil { return }
    le.ensureLayout()
    want := le.layout.Total
    if le.theta == nil || len(le.theta) != want { le.theta = make([]float64, want) }
    n := len(p)
    if n > want { n = want }
    copy(le.theta[:n], p[:n])
    for i := n; i < want; i++ { le.theta[i] = 0 }
    if le.PST != nil {
        off := 0
...
func passedPawns(b *Position) (uint64, uint64) {
    ...
    for x := b.White.Pawns; x != 0; x &= x - 1 {
        ...
        var mask uint64
        for rr := r + 1; rr <= 7; rr++ {
            if f > 0 { mask |= (fileMask(f-1) & (uint64(0xFF) << (rr*8))) }
            mask |= (fileMask(f) & (uint64(0xFF) << (rr*8)))
            if f < 7 { mask |= (fileMask(f+1) & (uint64(0xFF) << (rr*8))) }
        }
        if (b.Black.Pawns & mask) == 0 { wPassed |= (1 << uint(sq)) }
    }
    ...
    for x := b.Black.Pawns; x != 0; x &= x - 1 {
        ...
        if (b.White.Pawns & mask) == 0 { bPassed |= (1 << uint(sq)) }
    }
