...
func PawnStructDiffs(b *gm.Board) (mg [7]int, eg [7]int) {
    ...
    for x := wp; x != 0; x &= x - 1 {
        ...
        f := sq & 7
        var adj uint64
        if f > 0 { adj |= onlyFile[f-1] }
        if f < 7 { adj |= onlyFile[f+1] }
        if (wp & adj) == 0 { wIso++ }
    }
    for x := bp; x != 0; x &= x - 1 {
        sq := bits.TrailingZeros64(x)
    ...
    // Pawn levers: immediate capture opportunities
    wLeverCnt := bits.OnesCount64(wPawnAttackBB & b.Black.Pawns)
    bLeverCnt := bits.OnesCount64(bPawnAttackBB & b.White.Pawns)
...
func KingSafetyOneHot(b *gm.Board) (onehot [100]int) {
    ...
    add := func(white bool, innerCnt, outerCnt, innerW, outerW int) {
        ...
        if white { attackUnit[0] += u } else { attackUnit[1] += u }
    }
...
func WeakSquaresCounts(b *gm.Board) (weakDiff int, weakKingDiff int) {
    ...
    all := b.White.All | b.Black.All
    // Knights
    for x := b.White.Knights; x != 0; x &= x - 1 { sq := bits.TrailingZeros64(x); movementBB[0][0] |= KnightMasks[sq] }
    for x := b.Black.Knights; x != 0; x &= x - 1 { sq := bits.TrailingZeros64(x); movementBB[1][0] |= KnightMasks[sq] }
    // Bishops
    for x := b.White.Bishops; x != 0; x &= x - 1 {
        sq := bits.TrailingZeros64(x)
...
func BishopPairDiffsScaled(b *gm.Board) (mg int, eg int) {
    ...
    // Pawn attacks and levers for center state
    wp, bp := b.White.Pawns, b.Black.Pawns
    wPawnAttackBB := ((wp &^ bitboardFileA) << 7) | ((wp &^ bitboardFileH) << 9)
    bPawnAttackBB := ((bp &^ bitboardFileH) >> 7) | ((bp &^ bitboardFileA) >> 9)
    wLever, bLever := getPawnLeverBitboards(b, wPawnAttackBB, bPawnAttackBB)
...
func QueenInfiltrationCounts(b *gm.Board) (wInf int, bInf int) {
    ...
    for x := b.White.Knights; x != 0; x &= x - 1 { sq := bits.TrailingZeros64(x); movementBB[0][0] |= KnightMasks[sq] }
    for x := b.Black.Knights; x != 0; x &= x - 1 { sq := bits.TrailingZeros64(x); movementBB[1][0] |= KnightMasks[sq] }
    for x := b.White.Bishops; x != 0; x &= x - 1 { sq := bits.TrailingZeros64(x); movementBB[0][1] |= gm.CalculateBishopMoveBitboard(uint8(sq), all &^ PositionBB[sq]) }
    for x := b.Black.Bishops; x != 0; x &= x - 1 { sq := bits.TrailingZeros64(x); movementBB[1][1] |= gm.CalculateBishopMoveBitboard(uint8(sq), all &^ PositionBB[sq]) }
    for x := b.White.Rooks; x != 0; x &= x - 1 { sq := bits.TrailingZeros64(x); movementBB[0][2] |= gm.CalculateRookMoveBitboard(uint8(sq), all &^ PositionBB[sq]) }
    for x := b.Black.Rooks; x != 0; x &= x - 1 { sq := bits.TrailingZeros64(x); movementBB[1][2] |= gm.CalculateRookMoveBitboard(uint8(sq), all &^ PositionBB[sq]) }
    ...
    if (b.White.Queens & weakSquares[1] & ranksAbove[4] &^ bSpan) != 0 { wInf = 1 }
    // Black queen occupies enemy weak squares in enemy half, outside white span
    if (b.Black.Queens & weakSquares[0] & ranksBelow[4] &^ wSpan) != 0 { bInf = 1 }
    return
}
...
func BishopXrayCounts(b *gm.Board) (kDiff int, rDiff int, qDiff int) {
    ...
    for x := b.White.Bishops; x != 0; x &= x - 1 {
        ...
        if bb & b.Black.Kings  != 0 { kDiff++ }
        if bb & b.Black.Rooks  != 0 { rDiff++ }
        if bb & b.Black.Queens != 0 { qDiff++ }
    }
    // Black bishops
    for x := b.Black.Bishops; x != 0; x &= x - 1 {
        sq := bits.TrailingZeros64(x)
        ...
        bb := gm.CalculateBishopMoveBitboard(uint8(sq), occupied)
        if bb & b.White.Kings  != 0 { kDiff-- }
        if bb & b.White.Rooks  != 0 { rDiff-- }
        if bb & b.White.Queens != 0 { qDiff-- }
    }
    return
}
...
func CenterMobilityScales(b *gm.Board) (knDelta int, biDelta int) {
    ...
    for x := b.White.Pawns; x != 0; x &= x - 1 { sq := bits.TrailingZeros64(x); whiteFiles |= onlyFile[sq%8] }
    for x := b.Black.Pawns; x != 0; x &= x - 1 { sq := bits.TrailingZeros64(x); blackFiles |= onlyFile[sq%8] }
...
func ExtrasDiffs(b *gm.Board) (mg [7]int, eg [2]int) {
    ...
    for x := b.White.Rooks; x != 0; x &= x - 1 {
        ...
        if rookMoves & b.Black.Queens != 0 { rxCnt++ }
    }
    for x := b.Black.Rooks; x != 0; x &= x - 1 {
        sq := bits.TrailingZeros64(x)
        occupied := blackMask &^ PositionBB[sq]
        rookMoves := gm.CalculateRookMoveBitboard(uint8(sq), occupied)
        if rookMoves & b.White.Queens != 0 { rxCnt-- }
    }
    mg[3] = rxCnt
